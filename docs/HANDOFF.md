...
conversation.** Then pull the owner's current Figma selection (the **figma-desktop MCP is now wired**, loads at
session start — see the doc's "Figma access") and build the new options panel to the pixels. The owner has NEW bar
