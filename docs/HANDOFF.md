...
QA'd in-game each slice** (the owner is ruthlessly picky — see the pixel-perfect-every-pass memory). DONE + QA'd:
...
gaps to resolve with the owner: segments/border/2nd-color + the bar text/countdown features), then **Slice 5 = Texture
