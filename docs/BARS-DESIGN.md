...
Decided with the owner: **build Duration mode first**; UI polish comes from his Figma pass, so this
...
1. **Aura Duration** — a buff/debuff's remaining time (the owner's DoT timers). **← build first.**
...
## Type-aware editor (first step of the UI declutter — the owner's bloat concern)
...
display's kind — the pattern for the broader reorg. **the owner is designing the polished UI in Figma** (tokens
...
Plus a **type-aware editor** (Texture|Bar switch) — reconcile with the owner's Figma. Each pass ships + QAs on its own.
