...
> **Why a redesign:** the panel got bloated ([[avoid-ui-bloat]]). The owner mocked a cleaner IA in Figma.
> **Prototype is DEAD:** an HTML prototype was built this session (artifact `325ae79c`) and REJECTED by
> The owner as a "component dump" — the Aura-Type selector was duplicative and it showed no real workflow.
...
1. **Landing page ("Default State")** — on open with nothing selected: a big GA logo in the right pane, and
   ...
   **View All Auras** (de-emphasized, lower). Footer (shared everywhere): **Hide Blizzard's Cooldown Manager**
   CHECKBOX (the owner confirmed a checkbox here, NOT a switch) + **Profile: ‹name›** button.
...
**Duplicate Aura**, **Delete Aura**, **Group: ‹name›**. (New + Duplicate were added back per the owner 2026-07-09.)
...
## BARS — the crux (most important; the owner has NEW Figma mocks for this)
The owner's mocks in progress: **"aura bar (appearance, position & size)"** + **"bar aura type & source."** Build to them.

### There are exactly TWO bar types (the owner confirmed): **Duration** and **Stacks**
...
- **Trigger accordion (Icon):** Match ALL/ANY/NONE segmented control at top; condition rows `Spell [icon] = STATE`
  ...
  multiple spell names to add to group" (the owner's multi-select-to-group, chosen over drag-and-drop).
...
- **Workflow:** ask the owner to select the frame(s); `get_metadata` handles multiple (lists node IDs), but
...
- Four frames from session 8 (icon path, for reference): Default State `395:98`, Aura Trigger OPEN `397:631`,
  ...
  get the owner's current selection next session.**
...
- Suggested approach: build the new panel behind a flag / new entry first if helpful, but the owner wants it pixel-
...
## GAMMA (do not relitigate) — WoW `SetColorTexture` renders ~11/255 DARKER per channel on the owner's display
Figma navy `#060714` was fed to the panel base as **`#12131F`** (`COLOR.dark` in Core.lua) so it renders as `#060714`.
If a dark fill looks too dark, add ~11 per channel. Accent colors (mid-tone) were left as raw hex (the owner approved).
...
## Sections DONE + QA'd
- **Landing (slice 1)** ✅ QA'd. Logo `Media/ga_logo_full.png` (monogram+wordmark, the owner's), 3 create buttons, View All, footer (Hide-CDM checkbox + Profile two-weight button), dividers, small X (kept per the owner).
- **Appearance, Position & Size (slice 2)** ✅ QA'd — `C:BuildAppearanceSection(ct)`: texture field (placeholder "Leave blank to adopt the first trigger's icon") + Choose pill; Recolor(check+swatch)+Desaturate; Blend Mode/Strata pills; Alpha/Width/Height (aspect-linked, **padlock at x≈48 between W/H — mock wants a CHAIN-LINK icon; need a `link` PNG from the owner**)/X/Y sliders.
...
- **Text (slice 3b)** ✅ QA'd. `C:BuildTextSection(ct)`: content field (heroic-8%, 13px, placeholder=aura name); **Show Text + Show Charge Count** toggles (`makeToggle`); Font pill (opens `OpenFontPicker`); Text Color; **Size 6–300** (raised from 48 per the owner); Outline/Anchor dropdowns; X/Y. Non-seeding reads (`txt()`/`ensure()`). NOTE: mock's toggle row shows ONE toggle between two labels → built TWO toggles; **shortened "Show Current Charge Count (if applicable)" → "Show Charge Count"** to fit the row (confirm with the owner). "Show Text Above" label kept from mock (a bit ambiguous vs Anchor — flag).
- **Effects & Motion (slice 3c)** ✅ QA'd. `C:BuildEffectsSection(ct)` — **Glow only** (Type dropdown + Custom Color, reuses cfg.glow engine). **Custom Color moved to its OWN row** under the dropdown (the checkbox+label+swatch ~157px overran the 360 editor beside the 220 dropdown → swatch fell off the edge; height 92→112). **Motion PARKED.** No dedicated mock.
- **Sounds (slice 3d)** ⏳ built, PENDING FINAL QA. `C:BuildSoundSection(ct)` — sound-name pill + Test + a **"Play:" timing dropdown** (When it triggers / When it wears off / Pandemic window). The ENGINE is SHIPPED + committed (`55a2d97`, CDM.lua) and QA'd in-game (trigger + wear-off + pandemic-on-Agony). Section height 40→110. See the **sound-trigger-timings** memory for the full engine + pandemic-per-spell caveat.
- **Aura Load Conditions (slice 3e)** ⏳ built, PENDING FINAL QA. `C:BuildLoadConditionsSection(ct)` — inline visibility gate reusing the `cfg.visibility` model + `CDM:VisibilityGate` (Combat/Target dropdowns, 11 context toggles in 2 cols, spec multi-select, known-spell box, master Disabled/Enabled switch). NON-seeding reads. No open-state mock (`411:164` = header only) → styled to the language. Height 420. **Replaced the old per-aura "Visibility…" drawer button** (the drawer still backs GROUP load rules). ⚠ Every row here needs BOTH `refresh`+`setEnabled` (see Learnings — a missing one crashed `SetSelected`).
- **Editor now SCROLLABLE** ⏳ built, PENDING QA. The right pane is a **ScrollFrame + scroll child**; a draggable purple scrollbar in the 20px right margin (editor ends x=600, panel=620) shows only on overflow, wheel scrolls too. Needed because Load Conditions (~420px) exceeds the 614 pane. `C:SetEditorScroll` / `C:LayoutEditorScroll` (called at the end of `AccordionLayout`; sizes the scroll child + shows/hides the bar). Also fixed: the **left list was blank until a wheel-scroll** → `C:ShowEditor` now calls `RefreshList()` + `AccordionLayout()`. `ShowLanding` hides the scrollbar.

## NEXT (in order)
0. **⚠ FIRST: reconfirm the uncommitted-then-committed UI batch in-game.** Session-11 shipped (committed) but did NOT get final QA on: the **SetSelected crash fix**, **Sounds** section, **Load Conditions** section, **editor scrollbar**, **glow overflow fix**, **RefreshList-on-ShowEditor**. QA = `/reload` → click an existing aura → **Trigger section shows conditions + group controls (list headers/gears + green Group button) are back** (that was the crash symptom) → open **Load Conditions**, confirm it **scrolls inside the pane, doesn't spill into the footer**, scrollbar drags → glance at **Glow** Custom-Color on its own row. Watch for a **clipped dropdown menu** (see Learnings).
1. **Deferred polish pass:** remove left-pane "YOUR AURAS" header; nudge the whole editor up ~10px (mock content starts ~panel-y 31, currently ~40 — tied to `CONTENT_TOP`, shared with the left pane); swap the aspect **padlock → chain-link** icon (asset FOUND in Figma `397:351` "link-2"; still need it exported as a PNG from the owner); Appearance "few minor things" the owner parked.
2. **Slice 4 — BAR EDITOR** (type-specific; mocks `411:182` + `411:448`). Two bar types **Duration** / **Stacks** (Cooldown = a Duration reading a different clock). Bar has **NO multi-trigger builder** — its source IS its trigger. Wire the landing "Add Bar Aura" (currently makes a blank shell — no source) to: pick source spell (existing two-panel picker → `cfg.spellID`), Duration vs Stacks, Track aura|cooldown, drain|fill, Max/Unit(auto). **⚠ ENGINE-vs-MOCK GAPS to resolve with the owner FIRST:** the Bar Appearance mock shows **Segments / Segment Gap / Continuous|Segmented**, **Border**, and **Bar Color 1 + Bar Color 2** — the engine does NOT do segments/border/2nd-color yet (only orientation, drain/fill, one fill color + bg). Decide: build those engine features, or omit/grey them (the redesign rule = "only promise what works"). Also **bar TEXT features** the owner wants: (a) **style the stack-count text** — FEASIBLE + secret-safe (it's a FontString; not built), (b) **duration countdown NUMBER** — FEASIBLE-unbuilt via a hidden Cooldown widget w/ `SetHideCountdownNumbers(false)` styled by the cooldown's own font API (more limited than a FontString). Bar ENGINE (all 3 modes) is DONE + shipped; back-doors `/ga bar <id>` / `/ga bar cd <id>` / `/ga bar stacks <id> [max]` work NOW.
3. **Slice 5 — Texture editor** (infer from Icon; default-open Appearance, no trigger emphasis; often a decoration).

## Learnings this session (don't rediscover)
- **PIXEL-PERFECT EVERY PASS** — the owner is ruthlessly picky; never reuse an old-styled control or defer polish ("structure now, pixels later" made him angry). Pull the exact design context, apply it, get an in-game screenshot to compare. (memory: pixel-perfect-every-pass.)
...
- Assets added by the owner: `Media/ga_logo_full.png`, `Media/checkmark_white.png` (white → tint). Still needed: a **chain-link** PNG for the aspect lock.
- The DoT/bar lag ("stacks bar sort of laggy") is a SEPARATE, un-started investigation the owner flagged — not now.
- **Every `rows[#rows+1]` entry MUST have BOTH `refresh` AND `setEnabled`.** `SetSelected` (Config.lua ~798) runs `r:refresh(); r:setEnabled(cfg~=nil)` on EVERY row. A row missing either throws "attempt to call a nil value" and ABORTS the rest of SetSelected — which silently blanks the Trigger section (`TrigInlineRender`, line 802), hides the group controls (`RefreshGroupButton`, 801), and skips `RefreshList` (804). Cost a debugging round (the Load Conditions disable-switch row shipped without `setEnabled`; looked like a spec-switch bug, was a nil-method crash). Audit every new inline row.
- **Pandemic sound is PER-SPELL + target-only.** `C_CooldownViewer.GetValidAlertTypes(cooldownID)` must list `PandemicTime`, AND the CDM only arms it for TARGET auras (`GetAuraDataUnit()=="target"`). Unstable Affliction = noPANDEMIC (not a game pandemic DoT); Agony/Corruption/Haunt = yes. `/ga trace` now prints `alerts=[apply,remove PANDEMIC/noPANDEMIC]` per condition. **Self-buff "about to expire" is NOT reachable** (remaining time is secret; the curve trick gives a value you can render but not branch on for a sound; no CDM warning event for self-buffs). Details: sound-trigger-timings memory.
- **Dropdown-menu clip risk (UNFIXED, watch):** now that the editor is a ScrollFrame, a `MakeDropdown` menu opening while its control sits near the viewport BOTTOM can be clipped by the scroll area. Not hit in normal use (dropdowns sit high in their sections). If it bites: reparent the menu off the scroll child (keep the `SetPoint` anchor to the button, but parent the menu to the panel `p` so it isn't clipped).
