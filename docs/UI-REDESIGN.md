...
- **Effects & Motion (slice 3c)** ⏳ built, PENDING QA. `C:BuildEffectsSection(ct)` — **Glow only** (Type dropdown + Custom Color, reuses cfg.glow engine). **Motion is PARKED** (the owner: super low priority). No dedicated mock — styled to the language.
...
3. **Deferred polish pass:** remove left-pane "YOUR AURAS" header; nudge the whole editor up ~10px (mock content starts ~panel-y 31, currently ~40 — tied to `CONTENT_TOP`, shared with the left pane); swap the aspect **padlock → chain-link** icon (need a `link` PNG); Appearance "few minor things" the owner parked.
